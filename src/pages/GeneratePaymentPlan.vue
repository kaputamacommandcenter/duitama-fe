<template>
  <div class="p-8 min-h-screen">
    <h1 class="text-2xl font-semibold mb-6">Generate Tagihan</h1>
...
const fetchDiscounts = async () => {
  ...
  try {
    ...
    Swal.fire(
      "Error",
      "Gagal mengambil data Potongan Diskon dari server.",
      "error"
    );
  } finally {
...
const fetchIndividualPayers = async () => {
  ...
  try {
    ...
    Swal.fire(
      "Error",
      "Gagal mengambil data Payer Individu dari server.",
      "error"
    );
  } finally {
    isProcessing.value = false;
...
const fetchGroupPayers = async () => {
  ...
  try {
    ...
    Swal.fire(
      "Error",
      "Gagal mengambil data Kelompok Payer dari server.",
      "error"
    );
  } finally {
    isProcessing.value = false;
...
const fetchTemplates = async () => {
  try {
    ...
    Swal.fire(
      "Error",
      "Gagal mengambil data Template Pembayaran dari server.",
      "error"
    );
  } finally {
    isProcessing.value = false;
...
const createBulkPayload = () => {
  ...
  if (payerMode.value === "individual") {
    if (selectedPayers.value.length === 0) {
      return {
        success: false,
        message: "Pilih minimal 1 Payer Individu",
        payload: null,
      };
    }
    payers.value
    ...
    if (selectedGroups.value.length === 0) {
      return {
        success: false,
        message: "Pilih minimal 1 Kelompok Payer",
        payload: null,
      };
    }
    payerGroups.value
  ...
  if (finalPayers.length === 0) {
    return {
      success: false,
      message: "Tidak ada Payer yang valid untuk digenerate",
      payload: null,
    };
  }
  ...
  try {
    ...
    if (response.data.success) {
      ...
      Swal.fire({
        ...
        text:
          ...
          "Terjadi kesalahan saat menyimpan data secara bulk.",
        icon: "error",
      });
    }
  } catch (error) {
    ...
    Swal.fire({
      ...
      text:
        ...
        "Gagal terhubung ke server saat bulk insert.",
      icon: "error",
    });
  }
};
