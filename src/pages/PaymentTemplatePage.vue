    ...
    <div class="modal" :class="{ 'modal-open': modalVisible }">
      <div class="modal-box w-11/12 max-w-4xl">
        ...
        <div class="space-y-4 max-h-96 overflow-y-auto pr-2">
          <div
            v-for="(item, index) in currentTemplateDetails"
            :key="index"
          ...
          >
            <label class="form-control w-full md:w-5/12">
              ...
              <input
                type="text"
                v-model="item.billing_item_name"
            ...
            <div class="md:w-auto w-full">
              <button
                class="btn btn-error btn-sm btn-outline w-full md:w-auto"
...
const removeItem = (index) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire("Terhapus!", "Item tagihan berhasil dihapus.", "success");
    }
  });
};
...
const isFormValid = computed(() => {
  ...
  return currentTemplateDetails.value.every(
    (item) =>
      item.billing_item_name &&
      item.billing_item_name.trim() !== "" &&
      item.amount > 0
...
const saveTemplate = async () => {
  if (!isFormValid.value) {
    Swal.fire(
      "Validasi Gagal",
      "Harap lengkapi Nama Template, Nominal, dan semua Tanggal Jatuh Tempo dengan benar.",
      "warning"
    );
    return;
  }
  ...
  try {
    if (formMode.value === "add") {
      ...
      Swal.fire(
        "Berhasil!",
        `Template ${currentTemplate.value.name} berhasil ditambahkan.`,
        "success"
      );
    } else {
      await api.put(
        `/payment-templates/${currentTemplate.value.id}`,
        templatePayload
      );
      Swal.fire(
        "Berhasil!",
        `Template ${currentTemplate.value.name} berhasil diperbarui.`,
        "success"
      );
    }
